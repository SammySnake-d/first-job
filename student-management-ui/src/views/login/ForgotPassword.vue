<template>
        ...
        <div class="login-box">
          <div class="login-header">
            <div class="logo">
              ...
              <span class="logo-text">{{ currentStep === 0 ? '验证用户名' : '重置密码' }}</span>
            </div>
          </div>
          ...
          >
            ...
            <div class="form-actions">
              <el-button class="clear-btn" @click="handleBack">返回登录</el-button>
              <el-button 
                type="primary" 
                class="login-btn"
                :loading="loading"
          ...
          >
            ...
            <div class="form-actions">
              <el-button class="clear-btn" @click="currentStep = 0">上一步</el-button>
              <el-button 
                type="primary" 
                class="login-btn"
                :loading="loading"
            ...
            <el-button @click="toggleTheme">
              <el-icon>
                ...
                <svg v-else class="theme-icon" viewBox="0 0 24 24">
                  <path d="M12 18a6 6 0 1 1 0-12 6 6 0 0 1 0 12zm0-2a4 4 0 1 0 0-8 4 4 0 0 0 0 8zM11 1h2v3h-2V1zm0 19h2v3h-2v-3z"
                        fill="currentColor"/>
                </svg>
              </el-icon>
            </el-button>
  ...
  const handleVerify = async () => {
    ...
    try {
      loading.value = true
      await authService.verifyUsername(formData.value.username.trim())
      currentStep.value = 1  // 验证成功后进入下一步
    } catch ( error ) {
  ...
  /* 步骤条容器背景色 */
  .dark :deep(.el-steps--simple) {
    background: #1a1a1a;
  }
  
  /* 步骤条分隔线颜色 */
  .dark :deep(.el-step.is-simple .el-step__line) {
    background-color: #4c4c4c;
  }
  
  /* 已完成步骤的文字颜色 */
  .dark :deep(.el-step__title.is-success) {
    color: #67c23a;
  }
  ...
  .dark :deep(.el-step__title.is-process) {
    color: #e5e7eb;
  }
  
  .dark :deep(.el-step__title.is-wait) {
    color: #909399;
  }
